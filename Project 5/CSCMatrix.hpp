...
public:
  ...
  void worker_matvec(const Vector& x, Vector& y, unsigned long begin, unsigned long end) const {
    for (unsigned long i = begin; i < end; ++i) {
      for (unsigned long j = col_indices_[i]; j < col_indices_[i + 1]; ++j) {
  ...
  void matvec(const Vector& x, Vector& y, size_t partitions) const {
    ...
    for (size_t k = 0; k < partitions; ++k) {
      threads.push_back(
        std::thread(
          &CSCMatrix::worker_matvec, this, std::cref(x), std::ref(y), k * blocksize, (k + 1) * blocksize));
    }
    for (size_t k = 0; k < partitions; ++k) {
      threads[k].join();
    }
  ...
  void t_matvec(const Vector& x, Vector& y, size_t partitions) const {
    ...
    for (size_t k = 0; k < partitions; ++k) {
      threads.push_back(
        std::thread(
          &CSCMatrix::worker_t_matvec, this, std::cref(x), std::ref(y), k * blocksize, (k + 1) * blocksize));
    }
    for (size_t k = 0; k < partitions; ++k) {
      threads[k].join();
    }
