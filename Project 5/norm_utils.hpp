...
size_t find_10ms_size(F&& f, Args&&...) {
  Timer t;
...
size_t num_trials(size_t nnz) {
  ...
  double N_1k = std::ceil(5E8 / static_cast<double>(nnz));

  return 5 + static_cast<size_t>(N_1k);
}

template <typename Function>
void run_partitioned(Function&& f, size_t N_min, size_t N_max, size_t n_parts) {
  header(n_parts);
  Timer t;
...
template <typename Function>
void run_divide_and_conquer(Function&& f, size_t N_min, size_t N_max, size_t n_parts) {

  size_t log_n_threads = 0;
  for (size_t i = 1; i <= n_parts; i *= 2) {
    ++log_n_threads;
  }
  ...
  for (size_t size = N_min; size <= N_max; size *= 2) {
    ...
    for (size_t levels = 1; levels <= log_n_threads; ++levels) {
      t.start();
      for (size_t i = 0; i < ntrials; ++i) {
        norm1 = f(x, levels);
      }
